...
void setup(void) {
  ...
  //erial.print( F("Boot Vers: ") ); Serial.println(system_get_boot_version());
...
*/
  ...
  server.on("/timerMax", HTTP_GET, [](AsyncWebServerRequest *request) {
    sendTimerMax(); //Still maybe need to update this
  });

/*
  server.on("/timerMax", HTTP_POST, []() {
    String hours = server.arg("hour");
    String minutes = server.arg("min");
...
void sendTimerMax() //Do I need to update this to hours minutes??
{
